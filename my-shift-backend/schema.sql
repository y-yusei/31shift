...
CREATE TABLE IF NOT EXISTS shift_periods (
    ...
    created_by INTEGER, -- 作成者（管理者）のID
...
CREATE TABLE IF NOT EXISTS shift_submissions (
    id INTEGER PRIMARY KEY,
    period_id INTEGER NOT NULL, -- 提出期間のID
    user_id INTEGER NOT NULL, -- 提出者のID
    submission_date TEXT NOT NULL, -- 提出対象日（YYYY-MM-DD）
    start_time TEXT, -- 希望開始時間（HH:MM）
    end_time TEXT, -- 希望終了時間（HH:MM）
    break_time TEXT, -- 休憩時間（例：「60分」）
    notes TEXT, -- 備考
    status TEXT DEFAULT 'submitted' CHECK(status IN ('submitted', 'approved', 'rejected')),
    submitted_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    reviewed_at DATETIME,
    reviewed_by INTEGER, -- 承認者（管理者）のID
    review_notes TEXT, -- 管理者のコメント
    FOREIGN KEY (period_id) REFERENCES shift_periods(id),
    FOREIGN KEY (user_id) REFERENCES users(id),
    FOREIGN KEY (reviewed_by) REFERENCES users(id),
    UNIQUE(period_id, user_id, submission_date) -- 同じ期間・同じユーザー・同じ日付の重複を防ぐ
);
