...
// itty-routerのリクエスト型を拡張してenvを含める
...
router.get('/api/data', async (request) => {
	...
	try {
		...
		console.error("D1 Error in /api/data:", e.message);
		return new Response(`Database error: ${e.message}`, { status: 500 });
	}
});

// シフトを更新/作成/削除するAPI
router.post('/api/shift', async (request) => {
    try {
        const { env } = request;
        ...
        console.error("D1 Error in /api/shift:", e.message);
        return new Response(`Database error: ${e.message}`, { status: 500 });
    }
});

// 手動入力データを更新するAPI
router.post('/api/manuals', async (request) => {
	try {
        const { env } = request;
        ...
        console.error("D1 Error in /api/manuals:", e.message);
		return new Response(`Database error: ${e.message}`, { status: 500 });
	}
});
...
export default {
	async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
        ...
        try {
            ...
            response = new Response("Internal Server Error in router", { status: 500 });
        }
